...
from DONATE_ARMY_TG_MUSIC_PLAYER.core.call import DONATE_ARMY as DONATE_ARMY
from DONATE_ARMY_TG_MUSIC_PLAYER.misc import SUDOERS, db
...
from DONATE_ARMY_TG_MUSIC_PLAYER.utils.inline.play import stream_markup
from DONATE_ARMY_TG_MUSIC_PLAYER.utils.stream.autoclear import auto_clean
from DONATE_ARMY_TG_MUSIC_PLAYER.utils.thumbnails import get_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await DONATE_ARMY.pause_stream(chat_id)
        buttons = [
            [
                InlineKeyboardButton(
        ...
        await DONATE_ARMY.resume_stream(chat_id)
        buttons_resume = [
            [
                InlineKeyboardButton(
        ...
        await DONATE_ARMY.mute_stream(chat_id)
        ...
        await DONATE_ARMY.unmute_stream(chat_id)
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await DONATE_ARMY.st_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await DONATE_ARMY.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                await DONATE_ARMY.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_7"])
            button = stream_markup2(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
