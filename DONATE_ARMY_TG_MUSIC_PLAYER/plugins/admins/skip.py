...
from DONATE_ARMY_TG_MUSIC_PLAYER.utils.database import get_loop
...
async def skip(cli, message: Message, _, chat_id):
    if not len(message.command) < 2:
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await DONATE_ARMY.stop_stream(chat_id)
                except:
                    return
        except:
            try:
    ...
    if "live_" in queued:
        ...
        try:
            await DONATE_ARMY.skip_stream(chat_id, link, video=status)
        except Exception:
        ...
        try:
            await DONATE_ARMY.skip_stream(chat_id, file_path, video=status)
        except Exception:
            return await mystic.edit_text(_["call_7"])
        ...
        try:
            await DONATE_ARMY.skip_stream(chat_id, queued, video=status)
        except Exception:
            return await message.reply_text(_["call_7"])
        if videoid == "telegram":
            button = telegram_markup(_, chat_id)
